    ...
    def traverse_nodes(node_id=0,
        ...
        if node_id != 0:
            ...
            rule_condition = RuleCondition(feature_index=feature,
                                           threshold=threshold,
                                           operator=operator,
                                           support = tree["count"][node_id] / float(tree["count"][0]),
...
class RuleFit(BaseEstimator, TransformerMixin):
    ...
    ----------
        ...
        tree_generator: Optional: this object will be used as provided to generate the rules.
                        ...
                        Must be HistGradientBoostingRegressor or HistGradientBoostingClassifier, optional (default=None)
    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if 'r' in self.model_type:
            ...
            if not self.exp_rand_tree_size: # simply fit with constant tree size
                ...
                for i_size in np.arange(len(tree_sizes)):
                    ...
                    self.tree_generator.set_params(max_iter=curr_est_+1)
                    ...
                    self.tree_generator.get_params()['max_iter']
        ...
        print(f"Finished tree stuff in {time.time() - start} seconds")
        ...
        if self.rfmode=='regress':
            ...
            self.lscv=ElasticNet(
                n_lambda=n_alphas, alphas = alphas, n_splits=self.cv, max_iter=100000, cut_point=0,
            ...
            self.coef_=self.lscv.coef_[0]
            self.intercept_=self.lscv.intercept_
        print(f"Finished fitting CV linear model in {time.time() - start} seconds")
